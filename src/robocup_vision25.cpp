...
Mat robocup_vision25::Image_processing(const cv::Mat &img)
{
    ...
    if (cam_nice_point != 0) // 캠이 볼트래킹 모드일 경우 욜로 상태로 설정
    {
        ...
        if (cnt_ball != 0) // 공이 카운트 됐을 경우 실행
        {
            ...
            {
                ballPos = calcObjectDistance(pan_tilt.ptpos.TILT_POSITION, ROBOT_HEIGHT + TILT_L * (cos(pan_tilt.ptpos.TILT_POSITION * DEG2RAD) - 1), focalLen, prncPt, Point(ball_pts[i].x, ball_pts[i].y));
            }
            ball_pts.clear();
        ...
        if (cnt_line != 0) // 라인(특징점)이 카운트 됐을 경우 실행
        {
            ...
            {
                // 특징점의 거리 계산
                linePos = calcObjectDistance(pan_tilt.ptpos.TILT_POSITION, ROBOT_HEIGHT + TILT_L * (cos(pan_tilt.ptpos.TILT_POSITION * DEG2RAD) - 1), focalLen, prncPt, Point(pts[i].x, pts[i].y));
        ...
        if (cnt_robot != 0) // 로봇(obstacle)이 카운트 됐을 경우 실행
        {
            ...
            {
                robotPos = calcObjectDistance(pan_tilt.ptpos.TILT_POSITION, ROBOT_HEIGHT + TILT_L * (cos(pan_tilt.ptpos.TILT_POSITION * DEG2RAD) - 1), focalLen, prncPt, Point(robot_pts[i].x, robot_pts[i].y));
