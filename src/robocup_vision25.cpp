...
robocup_vision25::robocup_vision25() : Node("robocup_vision25")
{
  ...
  image_subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
    image_topic, 10, std::bind(&robocup_vision25::image_callback, this, std::placeholders::_1));
  ...
  calibration_info();
    ...
    timer_ = this->create_wall_timer(500ms, std::bind(&robocup_vision25::timerCallback, this));
}

robocup_vision25::~robocup_vision25()
{
  cv::destroyAllWindows();
}

void robocup_vision25::image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
{
...
Mat robocup_vision25::Image_processing(const cv::Mat& img)
{
    ...
    if(cam_nice_point != 0) //캠이 볼트래킹 모드일 경우 욜로 상태로 설정
    {
        ...
        for(std::vector<YOLOV4::YOLO_RESULT>::size_type i = 0; i < yolov4.vector_yolo.size();i++)
        {
        ...
        if(cnt_ball != 0) //공이 카운트 됐을 경우 실행
        {
            Rect result(0,0,0,0);
            double score = 0;
            for(std::vector<YOLOV4::YOLO_RESULT>::size_type i = 0; i < yolov4.vector_yolo.size();i++)//검출된 객체 수 만큼 반복 실행
            {
            ...
            for(std::vector<cv::Point_<float> >::size_type i = 0; i < ball_pts.size();i++)
            {
                ballPos = calcObjectDistance(pan_tilt.ptpos.TILT_POSITION, ROBOT_HEIGHT + TILT_L * (cos(pan_tilt.ptpos.TILT_POSITION*DEG2RAD) - 1), new_focalLen, new_prncPt, Point(ball_pts[i].x, ball_pts[i].y));
            }
            ball_pts.clear();
        ...
        if(cnt_line != 0) //라인(특징점)이 카운트 됐을 경우 실행
        {
            ...
            for(std::vector<cv::Point_<float> >::size_type i = 0; i < pts.size();i++)//저장된 데이터 수 만큼 반복
            {
                //특징점의 거리 계산
                linePos = calcObjectDistance(pan_tilt.ptpos.TILT_POSITION, ROBOT_HEIGHT + TILT_L * (cos(pan_tilt.ptpos.TILT_POSITION*DEG2RAD) - 1), new_focalLen, new_prncPt, Point(pts[i].x, pts[i].y));
        ...
        if(cnt_robot != 0) //로봇(obstacle)이 카운트 됐을 경우 실행
        {
            ...
            for(std::vector<cv::Point_<float> >::size_type i = 0; i < robot_pts.size();i++)
            {
                robotPos = calcObjectDistance(pan_tilt.ptpos.TILT_POSITION, ROBOT_HEIGHT + TILT_L * (cos(pan_tilt.ptpos.TILT_POSITION*DEG2RAD) - 1), new_focalLen, new_prncPt, Point(robot_pts[i].x, robot_pts[i].y));
